...
<template>
  ...
  >
    ...
    <div class="max-w-2xl mx-auto py-16 px-4 space-y-16 sm:px-6">
      <div
        v-for="(plan, mobilePlanIndex) in props.tiers"
        :key="mobilePlanIndex"
        class="border-t border-gray-200"
      >
        <div :class="[plan.featured ? 'border-emerald-600' : 'border-transparent', '-mt-px pt-6 border-t-2 sm:w-1/2']">
          <h3 :class="[plan.featured ? 'text-emerald-600' : 'text-gray-900', 'text-sm font-bold']">
            {{ plan.name }}
          </h3>
                ...
                >
                  ...
                  <dd class="flex items-center justify-end sm:px-4 sm:justify-center">
                    <span
                      ...
                      :class="[feature.tiers[mobilePlanIndex].featured ? 'text-emerald-600' : 'text-gray-900', 'text-sm font-medium']"
                    ...
                    <template v-else>
                      ...
                      <XIcon
                        v-else
                        class="mx-auto h-5 w-5 text-gray-400 opacity-50"
                        aria-hidden="true"
                      />
